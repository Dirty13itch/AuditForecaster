...
- **Google Calendar Integration:** Comprehensive multi-calendar integration with CalendarLayersPanel sidebar showing all user calendars with color indicators and toggle checkboxes (matching Google Calendar UI). Fetches events from multiple enabled calendars with automatic sync every 10 minutes (configurable via VITE_SYNC_INTERVAL_MS), syncs on page visibility change, and includes offline detection. Visual distinction between job-linked events (colored by status, 🔗 icon) and Google-only events (gray with 📅 icon, dashed border). One-click event-to-job conversion with ConvertGoogleEventDialog that pre-fills job data from event metadata (title→name, location→address, description→notes). Sync status indicators show real-time state (Syncing/Synced/Error/Offline) with automatic timeout cleanup. Calendar preferences persist toggle states in storage. Error resilience with sequential calendar fetching and partial-sync tolerance. GoogleEvent schema maintains audit trail with isConverted flag. **All-Day Event Support:** Properly handles all-day events using Luxon library for DST-aware timezone conversion (America/Chicago defaults to UTC-6 CST in winter, UTC-5 CDT in summer). Centralized `allDayDateToUTC()` helper with defensive error handling gracefully skips invalid dates without crashing sync. Extensive logging tracks all-day event processing and conversion for debugging. **Self-Healing Authentication:** Production-ready token refresh system with mutex-protected cache, comprehensive 401 error detection, automatic retry with fresh tokens, and user-friendly error messages. When OAuth tokens expire (typically after 1 hour), the system automatically refreshes via Replit's connector API and retries failed requests without user intervention. Promise mutex prevents concurrent refresh race conditions. All Google Calendar API methods (`fetchCalendarList`, `fetchEventsFromGoogle`, `syncEventToGoogle`, `deleteEventFromGoogle`) are wrapped with `withTokenRetry()` for automatic recovery. Extensive logging tracks cache hits, refreshes, auth errors, and retry attempts.
...
-   **luxon**: Date/time library with timezone and DST support for Google Calendar all-day event conversion.
